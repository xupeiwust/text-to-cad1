...
function createStepEntry({ repoRoot, rootPath, sourcePath, extension }) {
  ...
  const metadataPythonSource = artifact && artifactSourceKind !== "python"
  ...
  return {
    ...
    ...(sourceKind === "python" && pythonSourcePath ? {
      source: {
        ...
        ...((stepArtifact.sourceHash || metadataPythonSource?.sourceHash)
          ? { sourceHash: stepArtifact.sourceHash || metadataPythonSource.sourceHash }
          : {}),
        ...((stepArtifact.sourceFingerprint || metadataPythonSource?.sourceFingerprint)
          ? { sourceFingerprint: stepArtifact.sourceFingerprint || metadataPythonSource.sourceFingerprint }
          : {}),
      },
    } : {}),
...
export function scanCadDirectory({ repoRoot, rootDir = DEFAULT_VIEWER_ROOT_DIR, includePath = null } = {}) {
  if (!repoRoot) {
    throw new Error("repoRoot is required");
  }
