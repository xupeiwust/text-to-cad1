...
function parseJsonText(text) {
  ...
  if (!trimmed) {
    throw new Error("Clipboard does not contain STEP parameters");
  }
  try {
    ...
    throw new Error("STEP parameter paste must be JSON");
  }
}
...
export function parseStepModuleParamsPasteText(definition, text) {
  ...
  if (!isObject(rawValues)) {
    throw new Error("STEP parameter paste must be a JSON object");
  }
  ...
  if (unknownIds.length) {
    throw new Error(`Unknown STEP parameter${unknownIds.length === 1 ? "" : "s"}: ${unknownIds.join(", ")}`);
  }
  const count = Object.keys(values).length;
  if (!count) {
    throw new Error("No known STEP parameters found");
  }
  return { values, count };
}
