...
For STEP sidecars, name the sidecar `.<step-filename>.step.js` and infer the parent STEP from the sidecar location. Do not hardcode the STEP filename in the module.
...
- Hardcoding file paths or STEP names in sidecar modules instead of deriving them from colocated artifacts.
